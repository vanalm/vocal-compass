mock_provider "google" {
  ...
  mock_resource "google_monitoring_dashboard" {
    defaults = { id = "projects/123456789012/dashboards/dash-staging" }
  }
  ...
  mock_resource "google_cloud_run_v2_service" {
    defaults = { name = "vc-staging-app", location = "us-west1" }
  }
}
...
run "committed_settings_hold_no_email" {
  ...
  assert {
    condition     = length(regexall("[^\\s\"@]+@[^\\s\"@]+", file("${path.module}/staging.auto.tfvars"))) == 0
    error_message = "staging.auto.tfvars must hold no email address: alert_email goes in the gitignored terraform.tfvars"
  }
}
...
run "observability_wired" {
  ...
  assert {
    condition     = module.observability.events_dataset == "demo-project.vc_staging_events"
    error_message = "events dataset wiring"
  }
  assert {
    condition     = output.dashboard_url == "https://console.cloud.google.com/monitoring/dashboards/builder/dash-staging?project=demo-project" && output.events_dataset == "demo-project.vc_staging_events"
