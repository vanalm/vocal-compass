# Staging: the prod stack with deletion protection off. With domain = "" it
# serves on its run.app URL and skips the load balancer.
#
#   terraform init -backend-config="bucket=<project_id>-vc-tfstate"
#   terraform apply
#
# Needs bootstrap applied, a version on vc-staging-workos-api-key, and an image
# at app:bootstrap (terraform/README.md, First deploy, lists the commands).
